...
/**
 * Created by maddogS on 2017-04-13.
 */
function u64(h, l) {
    this.hi = h >>> 0;
    this.lo = l >>> 0;
...
function fromNumber(value) {
    if (isNaN(value) || !isFinite(value)) return u64.prototype.zero();
    var pow32 = 1 << 32;
...
var charString2bytes = function charString2bytes(s) {
    for (var b = [], i = 0; i < s.length; i++) {
        b[i] = s.charCodeAt(i);
    }return b;
};

var decimalNumStringToInt32Buffer = function decimalNumStringToInt32Buffer(s) {
    var r = new u256();
...
var hexNumStringToInt32Buffer = function hexNumStringToInt32Buffer(s) {
    var r = new u256();
    var i = 0;
    while (s.length) {
...
var bytes2Int32Buffer = function bytes2Int32Buffer(b) {
    var len = b.length;
    if (!len) return [];
    ...
    for (var pos = 8 - 1; pos >= 0; pos--) {
        if (this.u32[pos]) {
            for (var bits = 31; bits > 0; bits--) {
                if (this.u32[pos] & 1 << bits) return 32 * pos + bits + 1;
            }
            return 32 * pos + 1;
        }
    }
...
u256.prototype.shiftLeft = function (bits) {
    ...
    bits = bits % 32;
    for (var i = 0; i < 8; i++) {
        if (i + k + 1 < 8 && bits != 0) r.u32[i + k + 1] |= this.u32[i] >>> 32 - bits;
        if (i + k < 8) r.u32[i + k] |= this.u32[i] << bits;
    }
    return r;
};
...
u256.prototype.shiftRight = function (bits) {
    ...
    bits = bits % 32;
    for (var i = 0; i < 8; i++) {
        if (i - k - 1 >= 0 && bits != 0) r.u32[i - k - 1] |= this.u32[i] << 32 - bits;
        if (i - k >= 0) r.u32[i - k] |= this.u32[i] >>> bits;
    }
    return r;
};
